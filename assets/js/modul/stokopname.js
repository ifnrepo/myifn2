...
$("#tambahdata").click(function () {
    ...
    if (vl1 == '' && vl2 == '') {
        ...
        var kgs = $("#jmlkgs").val();
        if ($("#satuan option:selected").text().trim() == 'PCS' && $("#po").val() == '') {
            // var kgs = '';
        }
        var dp = $("#iddept").val();
        if (dp == 'GF' && $("#nobale").val() == '') {
...
$(document).on('click', "#editdataopname", function () {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $("#sku").val(ponodis(data[0]['po'], data[0]['item'], data[0]['dis'], data[0]['kode_brg']));
            $("#skubarang").val(ponodis(data[0]['po'], data[0]['item'], data[0]['dis'], data[0]['kode_brg']));
            $("#po").val(data[0]['po']);
    ...
    if (sat.trim() != 'SET') {
        if (jmlpcs != '' || jmlpcs != '0') {
            if (sat.trim() == 'PCS') {
                $("#pcsg").val(jmlpcs);
                if (po != '') {
                    var hasil = parseFloat($("#jmlpcs").val()) * parseFloat($("#kgsg").val());
        ...
        if ($("#item").val().includes("-1")) {
            $.when(hitungset()).then(function (data) {
                if (data.length > 0) {
                    if (jmlpcs != '' || jmlpcs != '0') {
                        var hasil = parseFloat($("#jmlpcs").val()) * parseFloat(data[0].berat);
                        $("#jmlkgs").val(rupiah(hasil, '.', ',', 3));
                    }
                } else {
...
function isidata(idx, tbx) {
    $.ajax({
        ...
        success: function (data) {
            ...
            $("#sku").val(ponodis(data[0]['po'], data[0]['item'], data[0]['dis'], data[0]['brgid']));
            $("#skubarang").val(ponodis(data[0]['po'], data[0]['item'], data[0]['dis'], data[0]['brgid']));
            $("#po").val(data[0]['po']);
            $("#item").val(data[0]['item']);
            $("#dis").val(data[0]['dis']);
            $("#jntb").val(data[0]['jntb']);
